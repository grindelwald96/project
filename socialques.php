...
cite{
 ...
 if (!$conn) {
    ...
    <?php
    $id=$_SESSION["id"];
    $sql="SELECT * FROM userquestions WHERE category='s' AND visibility='1';";
    $result=mysqli_query($conn, $sql);
    while ($row=mysqli_fetch_assoc($result)) {
     ...
     $sql1="SELECT userid FROM userquestions WHERE questionid='$qid';";
      ...
      switch ($row["questiontype"]) {
        ...
        <div class='row'>
        <div class='col-sm-2'>
        <input type='radio' name='ans' value='$val'> $op<br>
        </div>";
         $row1=mysqli_fetch_assoc($result1);
         $op=$row1["optionvalue"];
         $val=$row1["optionid"]; 
        ...
        default: echo"<form class='form-inline' action='anspost.php' method='post'>
          <div class='form-group'>
            <input type='text' class='form-control' placeholder='Rating' name='ans'>
          </div>
          <button class='btn btn-primary' type='submit' name='submit' value='$qid'> submit</button>
           </form>";
           break;
